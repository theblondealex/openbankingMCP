...
function App() {
  ...
  const remove = async (id: string) => { if (confirm("Remove this connection and all of its local data?")) { await fetch(`/api/connections/${id}/remove`, { method: "POST", headers: { "x-csrf-token": csrf() } }); load(); } };
  return <main className="shell">
    ...
    <section className="hero"><div><p className="eyebrow">YOUR FINANCIAL DATA, ON YOUR MAC</p><h1>Private finance context<br /><em>for your AI.</em></h1><p className="lead">Connect read-only bank data, store it encrypted locally, and ask your MCP client better questions.</p></div><div className="hero-card"><span className="shield">⌁</span><strong>Encrypted by default</strong><p>Credentials stay in macOS Keychain. Your local cache is encrypted and owner-only.</p></div></section>
