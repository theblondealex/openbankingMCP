...
2. Download and run the macOS installer:
   ...
   ```sh
   curl -fsSL https://raw.githubusercontent.com/theblondealex/openbankingMCP/main/scripts/install.sh -o /tmp/openbankingmcp-install.sh
...
The installer creates `~/Library/Application Support/OpenBankingMCP`, installs
an isolated Python environment, and registers a per-user `launchd` service.
The dashboard starts at login and remains at `127.0.0.1:3847` only.
...
- OpenBankingMCP itself is local, but an MCP client can send the financial data
  it receives to that client’s model provider. Review the privacy settings and
  data policy of Codex, Claude, Cursor, or any other client before asking it to
...
This project is a fork of `tkom04/openbankingMCP`. Its `pyproject.toml` declares
MIT, but the upstream repository has no root `LICENSE`; this unresolved licence
clarification should be tracked before wider distribution.
