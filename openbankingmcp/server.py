...
from .security import SecretStore, SecretStoreUnavailable, Vault, credential_store_name, csrf_token, redact, system_secret_store
...
def default_store() -> Store:
    root = data_dir()
    return Store(root / "openbankingmcp.sqlite3", Vault.from_secret_store(system_secret_store()))


def truelayer_client(secret_store: SecretStore | None = None) -> TrueLayer:
    secret_store = secret_store or system_secret_store()
    return TrueLayer(secret_store.get("truelayer-client-id") or "", secret_store.get("truelayer-client-secret") or "")
...
def build_tools_list() -> list[dict[str, Any]]:
    ...
    return [_tool("list_connections", "List local connection status."), _tool("list_accounts", "List connected accounts with aliases and masked identifiers."), _tool("get_balances", "Return latest local balances.", {"account_id": {"type": "string"}}), _tool("get_transactions", "Return redacted booked and pending transactions with an explicit status; defaults to seven days and 25 results.", {**dates, "limit": {"type": "integer", "minimum": 1, "maximum": 100}}), *summaries, _tool("search_transactions", "Search redacted local booked and pending transactions.", {**dates,"query":{"type":"string","minLength":1}})]
...
def call_tool(store: Store, name: str, arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    if "scope_update_required" in statuses:
        raise McpConnectionError("scope_update_required: Update access for the affected account in the local dashboard at http://127.0.0.1:3847.")
...
def create_app(store: Store | None = None) -> FastAPI:
    ...
    async def privacy(request: Request) -> dict[str, Any]:
        session(request); return {"local_only": True, "retention_days": 365, "credentials": credential_store_name(), "financial_data": "encrypted local cache"}
    ...
    async def sync(connection_id: str, request: Request) -> dict[str, str]:
        check_csrf(request)
        from .sync import ScopeUpdateRequired, sync_connection
        secret_store=system_secret_store(); client=truelayer_client(secret_store)
        ...
        except ScopeUpdateRequired:
            raise HTTPException(409, "scope_update_required")
    ...
    async def remove_connection(connection_id: str, request: Request) -> dict[str, bool]:
        ...
        try:
            ...
            if credentials.get("refresh_token"):
                ...
                connection=next(item for item in store.connections() if item["id"] == connection_id)
                store.save_connection(connection_id,connection["provider"],connection["status"],credentials,{})
            client.delete_credential(credentials["access_token"])
        except KeyError:
            raise HTTPException(404, "connection not found")
        except ReauthorizationRequired:
            pass
        except ProviderUnavailable:
            raise HTTPException(503, "TrueLayer is temporarily unavailable; retry removal shortly.")
    ...
    def start_connection(connection_id: str | None = None) -> dict[str, str]:
        ...
        if (secret_store.get("truelayer-client-id") or "").startswith("sandbox-"):
        ...
        client = truelayer_client(secret_store)
        callback_uri=secret_store.get("truelayer-redirect-uri") or "http://127.0.0.1:3847/oauth/callback"
    ...
    async def callback(state: str, code: Optional[str] = None) -> Response:
        ...
        secret_store=system_secret_store(); client=truelayer_client(secret_store)
        token=client.exchange_code(code,secret_store.get("truelayer-redirect-uri") or "http://127.0.0.1:3847/oauth/callback")
        store.save_connection(connection_id,"Bank connection","connected",token,{})
        from .sync import ScopeUpdateRequired, sync_connection
        try:
            sync_connection(store, client, connection_id)
        except (ProviderUnavailable, ReauthorizationRequired, ScopeUpdateRequired):
            pass
...
def run_mcp_server() -> None:
    ...
    for line in sys.stdin:
        try:
            ...
            if method == "initialize": result = {"protocolVersion": "2024-11-05", "capabilities": {"tools": {}}, "serverInfo": {"name": "openbankingmcp", "version": __version__}}
...
def start() -> None:
    try:
        ...
        raise SystemExit(
            "OpenBankingMCP could not access the operating system credential store. Run openbanking-mcp-doctor for the exact fix."
        ) from None
