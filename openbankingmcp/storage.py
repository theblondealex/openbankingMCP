...
class Store:
    ...
    def upsert_transaction(self, item: dict[str, Any]) -> None:
        ...
        fields = (item["id"], item["account_id"], item["booked_on"], item["amount_minor"], item["currency"], item.get("merchant"), item.get("reference"), item["category"], raw, now())
        with self.connect() as con:
            con.execute("INSERT INTO transactions VALUES (?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET booked_on=excluded.booked_on,amount_minor=excluded.amount_minor,merchant=excluded.merchant,reference=excluded.reference,category=excluded.category,raw_payload=excluded.raw_payload", fields)
