"""Interactive, local-only TrueLayer credential setup."""
...
from .security import SecretStoreUnavailable, secret_store_help, system_secret_store
...
def main() -> None:
    try:
        ...
        raise SystemExit(f"No secure operating system credential store is available. {secret_store_help()}") from None
    print(f"OpenBankingMCP TrueLayer live setup (stores credentials in {secret_store.display_name}).")
    ...
    try:
        secret_store.set("truelayer-client-id", client_id)
        secret_store.set("truelayer-client-secret", client_secret)
        secret_store.set("truelayer-redirect-uri", DEFAULT_REDIRECT_URI)
    except SecretStoreUnavailable:
        raise SystemExit(f"The credential store could not save the credentials. {secret_store_help()}") from None
    print(f"Saved live credentials to {secret_store.display_name}. The local dashboard is http://127.0.0.1:3847.")
