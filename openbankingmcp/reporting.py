...
def summarise(items: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {"spending_minor": sum(-item["amount_minor"] for item in spending), "income_minor": sum(item["amount_minor"] for item in items if item["amount_minor"] > 0), "by_category_minor": dict(sorted(totals.items()))}
